...
def free_for(schedule, next_free):
    items = get_items_sorted_by_start_times(schedule)
    for item in items:
        if (item['status'] == 'busy') or (treat_tentative_as_busy and item['status'] == 'tentative'):
            if parse_start_time(item) > next_free:
                return parse_start_time(item) - next_free
    return "Forever"
...
def get_out_of_office_events(working_hours_dict, start_time, end_time):
    enter_office = parse_working_hours_time(working_hours_dict['startTime'],working_hours_dict['timeZone'])
    leave_office = parse_working_hours_time(working_hours_dict['endTime'],working_hours_dict['timeZone'])
    # Need to create events between start_time and end_time which consist of working hours
